...
```bash
docker build -f fruitAGNTCY/fruit_agents/lungo/docker/Dockerfile.auction-supervisor -t auction-supervisor .
```
...
#### Build the Docker Image
```bash
docker build -f fruitAGNTCY/fruit_agents/lungo/docker/Dockerfile.ui -t ui-server .
```
